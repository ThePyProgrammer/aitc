    ...
    >
      <div className="bg-surface-container p-6 max-w-md w-full border border-outline-variant/40 shadow-2xl shadow-black/50">
        ...
        <p className="font-body text-sm mb-6 text-on-surface-variant">
          Install the AITC PreToolUse hook in this repo's <code>.claude/settings.local.json</code>
          {' '}so AITC can gate tool calls from this agent? The change applies to future sessions;
          the currently-running agent is unaffected.
        </p>
        <div className="flex gap-2 justify-end">
          <button
            ...
            disabled={busy}
            onClick={decline}
            className="px-4 py-2 bg-surface-container-high font-headline text-[10px] font-bold uppercase tracking-widest disabled:opacity-50"
          >
            DECLINE
          </button>
          <button
            ...
            disabled={busy}
            onClick={accept}
            className="px-4 py-2 bg-primary text-on-surface font-headline text-[10px] font-bold uppercase tracking-widest disabled:opacity-50"
          >
            ACCEPT
          </button>
        </div>
