...
export function ToolUseCard({ event }: ToolUseCardProps) {
  ...
  return (
    <motion.div
      ...
      className="border border-outline bg-surface-container-highest rounded-sm mx-5 my-1.5"
      transition={{ duration: 0.12, ease: 'easeOut' }}
    >
      <button
        type="button"
        onClick={() => setExpanded((prev) => !prev)}
        className="w-full flex items-center gap-2 px-4 py-1.5 text-left hover:bg-surface-bright transition-colors"
      ...
      <AnimatePresence>
        {expanded && (
          ...
          >
            <div className="px-4 pb-3 pt-1 bg-surface-container-highest border-t border-outline">
              <ToolPreview
                toolName={toolName ?? ''}
