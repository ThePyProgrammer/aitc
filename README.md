...
Wave 2 is where the scope creep lives and we're in it. **Phase 11** shipped with d3-force in a dedicated Worker and "damn responsive" force-config sliders as the live witness. **Phase 11.1** (the zoom-scroll lag that Phase 11 exposed) shipped too — but only after the manual smoke surfaced two fresh regressions that both got patched same-session. Regression #1: extreme zoom-in was blanking main canvas AND minimap with zero recovery — pan, zoom-out, and force-config edits all inert — NaN/Infinity propagating through `ctx.setTransform` into a silent no-op. Fixed via defensive `Number.isFinite` guards at both `useCanvasZoomPan.setViewport` and `radarStore.setViewport`; user re-smoke confirmed the blanking is gone. Regression #2: cold-boot could wedge on the `BUILDING GRAPH` loader until pause→resume monitoring unstuck it — root-cause candidate is React StrictMode double-invoke leaving `useGraphLayout.lastIdsRef` populated from pass-1 so pass-2's fresh worker fails both the `isFirst` and `rewarm` gates and never receives init; fixed by resetting the ref in the worker-lifecycle cleanup (one-off symptom, defensive fix, awaiting natural observation). The earlier `e9d999b` reversal still stands — rAF wheel coalescer added 2-rAF pipeline latency, shipped approach is the targeted `drawFolderHulls` cache fix (`ebfa8ab`) + visibilitychange dirty-trigger. Low-zoom perf lag (zoom-in from fully-zoomed-out feels heavy; smooth once you're already zoomed in) deferred to a future Phase 11.2.

Wave 3 collects the reality checks — things that only break once you actually run the app with multiple long-lived agents. None of these four are blocked by Wave 2; all could be picked up in parallel. **Phase 18** (registry flooding) shipped 2026-04-21 — the hybrid `cwd-in-repo + parent-PID-in-candidate-set` filter in 18-01, `MAX_AGENTS` raised to 1000 with a doc comment formalising the D-03 rationale, and a new `get_registry_stats` Tauri command plus `capacity_hits_since_start: AtomicU64` counter for post-hoc debugging. 18/18 automated gates green; the optional 10-minute live-soak witness was accepted as non-blocking and deferred — it'll run opportunistically the next time concurrent agents are deployed. **Phase 19** (chat-transcript polish) is now 4/4 code-complete — the `TurnBuffer` aggregator coalesce (D-01), `SessionStart` hook envelope drop (D-04), `MarkdownBody` with `react-markdown` + `remark-gfm` + shiki code-fences (D-03), and `selectToolUseWithResult` + per-tool summary dispatcher + status dot on `ToolUseCard` (D-02) all landed this session. 21/21 V-19 Nyquist assertions pass. Three human-UAT items remain in [`19-HUMAN-UAT.md`](.planning/phases/19-polish-phase-10-chat-transcript-rendering-four-related-gaps-/19-HUMAN-UAT.md): codey-visual parity on tool-use cards, SessionStart-noise absence in a live transcript, and markdown rendering under real streaming. Caught its own regression mid-phase — commit `339549d` moved the DB write to turn-boundary and broke progressive reveal for agents mid-turn (no frontend listener for `agent-assistant-delta`); filed as a known-issue note, not a blocker for the UAT sign-off. **Phase 17** (conflict-triggered gating) remains drafted; see [`17-CONTEXT.md`](.planning/phases/17-conflict-triggered-pretooluse-gating-replace-tool-category-g/17-CONTEXT.md) for the three unresolved design questions. **Phase 20** (diff-aware agent polling) was filed the same day I ran a codebase inefficiency audit and found the `agentStore` 2-second poll replaces the entire agent array wholesale — so every `AgentChannelList` / `TowerControl` / sidebar-badge subscriber re-renders on the heartbeat even when literally nothing about any agent changed. Scope is a per-agent diff emit (upsert changed, remove missing, keep untouched by reference) so Zustand's selector-level reference equality can let unchanged subscribers stay quiet. Perf-only; no schema change; no visual change.
